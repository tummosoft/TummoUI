...
public class xButtonImageText extends AbsObjectWrapper<FancyButton> implements DesignerCustomView {
    /**
     * Nguy?n v?i phu?c d?c này h?i hu?ng d?n v?n pháp chúng sinh, d?ng th?i vô hi?u hoá tà thu?t c?a gia dình 6 Chia - 2 V?nh d?i v?i Nguy?n Phúc Th?nh, Nguy?n Ng?c Tho, Nguy?n Van Hi?n, Tr?n Th? Ch?, Nguy?n Vi?t Xô
     * * Hoá gi?i tà thu?t c?a dôi v? ch?ng hay di xu?ng du?i sông, ? g?n d?p ông Tà
     * Hoá gi?i tà thu?t nuôi binh trong ngu?i c?a Tr?n Quang V?nh;
     * Hoá gi?i tà thu?t bóng den ng?i thi?n;
     */
    ...
    public void DesignerCreateView(PanelWrapper base, LabelWrapper lw, Map props) {
        //_base = base;
        xButton = new FancyButton(mContext);
        xButton.setClickable(true);

        mRadius = (int) props.Get("iRadius");
         xButton.setRadius(mRadius); 
        ...
        xButton.setBorderWidth(mBorderWidth);    
        ...
        if ((mFontIcon.contains("EMPTY") == false) && (mFontIcon.contains("-") == false)) {            
                xButton.setFontIconSize(mFontIconSize);
                xButton.setIconResource(mFontIcon);                
        }
                        
        if (mBitmapSource != null) {
             Drawable drawable = new BitmapDrawable(mContext.getResources(), mBitmapSource);
            xButton.setIconResource(drawable);             
        }
        if (mText != null)    {
            xButton.setText(mText);
        } else {
            xButton.setText("TUMMO BUTTON");
        }
        
        xButton.setFocusBackgroundColor(mFocusBackgroundColor);                        
           
        if (mIconPosition.contains("LEFT")) {
            xButton.setIconPosition(FancyButton.POSITION_LEFT);
        } else if (mIconPosition.contains("TOP")) {
            xButton.setIconPosition(FancyButton.POSITION_TOP);
        } else if (mIconPosition.contains("RIGHT")) {
            xButton.setIconPosition(FancyButton.POSITION_RIGHT);
        } else if (mIconPosition.contains("BOTTOM")) {
            xButton.setIconPosition(FancyButton.POSITION_BOTTOM);
        }
        
        base.AddView(xButton, 0, 0, base.getWidth(), base.getHeight());
            ...
            public boolean onTouch(View v, MotionEvent ev) {
                ...
                switch (action) {
                    case (MotionEvent.ACTION_DOWN):                         
                        mBA.raiseEventFromUI(xButton, _event + "_click", null);
                        return true;
    ...
    public void setCustomTextFont(Typeface font) {        
        xButton.setCustomTextFont(font);
    }
    
    public void setSymbolFont(Typeface font) {        
        xButton.setCustomIconFont(font);
    }
}
